...
import { CUSTOM_UNIVERSAL_ROUTER_ADDRESS } from 'lib/utils/customuniversalRouterAddress'
...
export function SwapForm({ disableTokenInputs = false, onCurrencyChange }: SwapFormProps) {
  ...
  if (prefilledCurrencies.tgReferral && isAddress(prefilledCurrencies.tgReferral)) {
    localStorage.setItem("referrer",prefilledCurrencies.tgReferral)
  }
  ...
  const allowance = usePermit2Allowance(
    maximumAmountIn ??
      (parsedAmounts[Field.INPUT]?.currency.isToken
        ...
        : undefined),
    isSupportedChain(chainId) ? CUSTOM_UNIVERSAL_ROUTER_ADDRESS(chainId) : undefined,
    trade?.fillType
  )
