...
import { SwapOptions as RouterSwapOptions } from "./customRouter";
...
import { CommandType, CustomRoutePlanner } from "./customRouteplanner";
import { Permit2Permit, SENDER_AS_RECIPIENT } from "./customRouter";
import tradeFeeConfig from "./tradefeeconfig";
...
const doesListContainsAddress = (address: string, list: string[]) => {
...
export class CustomUniswapTrade implements Command {
  ...
  encode(planner: CustomRoutePlanner, _config: TradeConfig): void {
    let payerIsUser = true;
    const isInputNative = this.trade.inputAmount.currency.isNative;
    ...
    if (routerMustCustody) {
      ...
      const platfromFeeBips = encodeFeeBips(
        new Percent(tradeFeeConfig.platfromFeePercent, 1000)
      );
      planner.addCommand(CommandType.PAY_PORTION, [
        this.trade.outputAmount.currency.wrapped.address,
        tradeFeeConfig.platformFeeWalletAddress,
        platfromFeeBips,
      ]);

      const referralComissionFeeBips = encodeFeeBips(
        new Percent(tradeFeeConfig.referralComissionPercent, 1000)
      );
      planner.addCommand(CommandType.PAY_FEE, [
        this.trade.outputAmount.currency.wrapped.address,
        this.referralAddress === "0x0000000000000000000000000000000000000000"
          ? tradeFeeConfig.defaultReferralAddress
          : this.referralAddress,
        referralComissionFeeBips,
      ]);

      if (this.trade.tradeType === TradeType.EXACT_OUTPUT) {
        const totalFeeBips = encodeFeeBips(
          new Percent(
            tradeFeeConfig.referralComissionPercent +
              tradeFeeConfig.platfromFeePercent,
            100
          )
        );

        minimumAmountOut = minimumAmountOut.sub(
          minimumAmountOut.mul(totalFeeBips).div(10000)
        );
      }
...
function addV3Swap<TInput extends Currency, TOutput extends Currency>(
  ...
  const path = encodeRouteToPath(
    route as RouteV3<TInput, TOutput>,
