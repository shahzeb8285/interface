...
export function queryParametersToCurrencyState(parsedQs: ParsedQs): SerializedCurrencyState {
  const { tgReferral, tgTokenA, tgTokenB } = getTgParams(parsedQs.tgWebAppStartParam)

  let inputCurrency = parseCurrencyFromURLParameter(tgTokenA?tgTokenA:parsedQs.inputCurrency)
  let outputCurrency = parseCurrencyFromURLParameter(tgTokenB?tgTokenB:parsedQs.outputCurrency)
...
const getTgParams = (tgParams: any) => {

  if (!tgParams) {
    return {
      tgReferral: null, tgTokenA: null, tgTokenB: null
    }
  }
  const formattedData = tgParams.replaceAll("-", "=").replaceAll("_", "&")
  const formattedUrl = `https://localhost?${formattedData}`
  const rewReferral = getQueryParams("seeder", formattedUrl)
  let referral;
  if (rewReferral && isAddress(rewReferral)) {
    referral = rewReferral
  }

  const tokenA = getQueryParams("tokenA", formattedUrl)
  const tokenB = getQueryParams("tokenB", formattedUrl)
  return { tgReferral: referral, tgTokenA: tokenA, tgTokenB: tokenB }
}
