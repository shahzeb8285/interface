...
export const StatWrapper = styled.div<{ isInfoTDPEnabled?: boolean }>`
  ...
  flex: 1;
  padding-top: 24px;
  padding-bottom: ${({ isInfoTDPEnabled }) =>
    isInfoTDPEnabled ? "0px" : "24px"};
...
export function ComissionTable() {
  ...
  //@ts-ignore
  const { data } = useReferralComissionQueryQuery(
    account ? account : "",
    connectedChainId
  );
  ...
  const tableColumns = useMemo(() => {
    ...
    return [
      columnHelper.accessor((row) => row.timestamp, {
        ...
        header: () => (
          <Cell justifyContent="flex-start" width={240} grow>
            <ThemedText.BodySecondary>Date</ThemedText.BodySecondary>
          </Cell>
        ),
        cell: (timestamp) => (
          <Cell justifyContent="flex-start" width={240} grow>
      ...
      columnHelper.accessor((row) => row.tokenSymbol, {
        ...
        header: () => (
          <Cell justifyContent="flex-start" width={240} grow>
            <ThemedText.BodySecondary>Token</ThemedText.BodySecondary>
          </Cell>
        ),
        cell: (tokenSymbol) => (
          <Cell justifyContent="flex-start" width={240} grow>
      ...
      columnHelper.accessor((row) => row.transactionHash, {
        ...
        header: () => (
          <Cell justifyContent="flex-start" width={240} grow>
            <ThemedText.BodySecondary>Transaction</ThemedText.BodySecondary>
          </Cell>
        ),
        cell: (hash) => {
          const rawHash = hash.getValue?.();
          const finalHash = `${rawHash.slice(0, 5)}....${rawHash.slice(
            rawHash.length - 5,
            rawHash.length
          )}`;
          return (
            <Cell justifyContent="flex-start" width={240} grow>
              <ExternalLink
                href={getExplorerLink(
                  connectedChainId ? connectedChainId : 1,
                  hash.getValue?.(),
      ...
      columnHelper.accessor((row) => row, {
        ...
        header: () => (
          <Cell justifyContent="flex-start" width={240} grow>
            <ThemedText.BodySecondary>From</ThemedText.BodySecondary>
          </Cell>
        ),
        cell: (row) => {
      ...
      columnHelper.accessor((row) => row, {
        ...
        header: () => (
          <Cell justifyContent="flex-start" width={240} grow>
            <ThemedText.BodySecondary>Amount</ThemedText.BodySecondary>
          </Cell>
        ),
        cell: (row) => {
          const amount = row.row.original.amount;
          const decimal = Number(row.row.original.tokenDecimal);
          const finalTokenAmount = ethers.utils
  ...
  return (
    <Container>
      <StatsWrapper data-testid="token-details-stats">
        <Header isInfoTDPEnabled={true}>Seeding Details</Header>
        <TokenStatsSection>
          <Container>
            <Stat
              ...
              description={
                " text ever since the 1500s, when an unknown printer took a galley oft"
              }
