...
import { CustomSwapRouter } from 'lib/utils/customRouter'
...
export function useUniversalRouterSwapCallback(
  ...
  const { referralAddress } = useMyReferralAddress()
  ...
  return useCallback(async () => {
    return trace('swap.send', async ({ setTraceData, setTraceStatus, setTraceError }) => {
      try {
        ...
        const { calldata: data, value } = CustomSwapRouter.swapERC20CallParameters(trade, {
        ...
        const tx = {
          from: account,
          to: CUSTOM_UNIVERSAL_ROUTER_ADDRESS(chainId),
          data,
